import { errorKind, errorMessage, type HeliconClient } from "../client";
import type {
  ApprovalMode,
  ApprovalRequest,
  AttachmentView,
...
import { NotificationManager, type Notifier } from "./notify";
import { UpdateManager, type AppUpdater } from "./updates";
...
export class HeliconController {
  ...
  private takePlanUsage(usage: import("../types").PlanUsage, accountId: string | null = null): void {
  ...
  listDirectory(path: string): Promise<import("../types").DirectoryListing> {
  ...
  usageReport(days: number): Promise<import("../types").UsageReport> {
  ...
  private addShellRun(sessionId: string, run: import("../types").ShellRun): void {
    this.update((s) => {
      const thread = s.threads[sessionId];
  ...
  sendShellOutput(sessionId: string, run: import("../types").ShellRun): Promise<boolean> {
